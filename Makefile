...
DB_NAME=$(APP_NAME)-db
GO=$(shell which go)
# Role criada pelo ADMIN da AWS
ACCESS_ROLE_ARN=arn:aws:iam:$(ACCOUNT_ID):role/AppRunnerECRAccessRole


# Creating a Security Group
create-sg:
	@if ! aws ec2 describe-security-groups --filters "Name=group-name, Values=$(SG_NAME)" --region $(REGION) --query 'SecurityGroups[0].GroupId' --output text | grep -qE '^sg-' \; then \
